...
public class BasePage implements Page {
    ...
    public List<ConsoleMessage> getErrorMessages() {
        List<ConsoleMessage> errorMessages = new ArrayList<>();
        page.onConsoleMessage(consoleMessage -> {
            if (consoleMessage.type().equalsIgnoreCase("error")) {
                errorMessages.add(consoleMessage);
            }
        });
        return errorMessages;
    }

    public void reportConsoleErrors(Level level) {
        List<ConsoleMessage> errorMessages = getErrorMessages();
        if (!errorMessages.isEmpty()) {
            logger.atLevel(level).log("Found console errors: " + errorMessages);
        }
    }
}
